...
class BaseOptions():
    ...
    def initialize(self):
        self.parser.add_argument('--name', type=str, required=True, help='Name of this trial')

        self.parser.add_argument('--vq_name', type=str, help='Name of the rvq model.')

        self.parser.add_argument("--gpu_id", type=int, default=-1, help='GPU id')
        self.parser.add_argument('--dataset_name', type=str, default='["pdgam"]', help='JSON list of datasets, e.g. ["pdgam"]')
    ...
    def parse(self):
        ...
        self.opt.model_stage = self.model_stage

        if self.opt.gpu_id != -1:
            # self.opt.gpu_id = int(self.opt.gpu_id)
            torch.cuda.set_device(self.opt.gpu_id)

        if isinstance(self.opt.dataset_name, str):
            self.opt.dataset_name = json.loads(self.opt.dataset_name)
        args = vars(self.opt)
        ...
        if self.is_train:
            # save to the disk
            self.opt.db_save_name = ''
            for item in self.opt.dataset_name:
                self.opt.db_save_name = self.opt.db_save_name + item + '_'
            self.opt.db_save_name = self.opt.db_save_name[:-1]
            expr_dir = os.path.join(self.opt.checkpoints_dir, self.opt.db_save_name, self.opt.name)
