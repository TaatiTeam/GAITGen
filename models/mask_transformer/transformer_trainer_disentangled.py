...
from utils.eval_t2m import evaluation_Dmask_transformer, evaluation_Dres_transformer
...
class DMaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, vq_model):
        ...
        if args.is_train:
            wandb.init(project="VQ_PD_Generation", name=self.opt.name, config=vars(self.opt))
    ...
    def prepare_vq_input(self, motions, severity_labels):
        if self.opt.vq_conditional:
            return (motions, severity_labels.to(self.device))
        else:
            return (motions,)

    def forward(self, batch_data):
        conds, motion, m_lens, label = batch_data
        motion = motion.detach().float().to(self.device)
        m_lens = m_lens.detach().long().to(self.device)
        inp = self.prepare_vq_input(motion, label)

        # (b, n(number of codebooks), q(6layers))
        
        m_code_idx, d_code_idx, _, _, _, _ = self.vq_model.encode(*inp)
        ...
        # self.pred_ids = []
        # self.acc = []
        _loss, _pred_ids_m, _pred_ids_d, _acc = self.t2m_transformer(m_code_idx[..., 0], d_code_idx[..., 0], conds, m_lens, m_lens)  # m_lens = d_lens
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        if self.opt.SFO:
            self.opt_t2m_transformer = schedulefree.AdamWScheduleFree(self.t2m_transformer.parameters(), lr=self.opt.lr, weight_decay=5e-2)
            self.opt_t2m_transformer.eval()
        else:
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching = evaluation_Dmask_transformer(
        ...
        while epoch < self.opt.max_epoch:
            ...
            self.vq_model.eval()
            if self.opt.SFO:
                self.opt_t2m_transformer.train()
            ...
            for i, batch in enumerate(train_loader):
                ...
                logs['Train/acc'] += acc
                logs['Train/lr'] += self.opt_t2m_transformer.param_groups[0]['lr']
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching = evaluation_Dmask_transformer(
                self.opt.save_root, eval_val_loader, self.t2m_transformer, self.vq_model, epoch, best_fid=best_fid,
...
class DResidualTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        ce_loss, pred_ids, acc = self.res_transformer(m_code_idx, d_code_idx, conds, m_lens)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        if self.opt.SFO:
            self.opt_res_transformer = schedulefree.AdamWScheduleFree(self.res_transformer.parameters(), lr=self.opt.lr, weight_decay=5e-2)
            self.opt_res_transformer.eval()
        else:
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching = evaluation_Dres_transformer(
            self.opt.save_root, eval_val_loader, self.res_transformer, self.vq_model, epoch,
            best_fid=100, best_div=100,
            best_top1=0, best_top2=0, best_top3=0,
            best_matching=100, eval_wrapper=eval_wrapper,
            plot_func=plot_eval, save_ckpt=False, save_anim=False, vq_conditional=self.opt.vq_conditional
        )
        best_loss = 100
        best_acc = 0
        ...
        while epoch < self.opt.max_epoch:
            ...
            self.vq_model.eval()
            if self.opt.SFO:
                self.opt_res_transformer.train()
            ...
            for i, batch in enumerate(train_loader):
                ...
                logs['Train/loss'] += loss
                logs["Train/acc"] += acc
                logs['Train/lr'] += self.opt_res_transformer.param_groups[0]['lr']
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching = evaluation_Dres_transformer(
                self.opt.save_root, eval_val_loader, self.res_transformer, self.vq_model, epoch, best_fid=best_fid,
                best_div=best_div, best_top1=best_top1, best_top2=best_top2, best_top3=best_top3,
                best_matching=best_matching, eval_wrapper=eval_wrapper,
                plot_func=plot_eval, save_ckpt=True, save_anim=(epoch%self.opt.eval_every_e==0), vq_conditional=self.opt.vq_conditional
            )