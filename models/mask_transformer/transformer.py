...
class MaskTransformer(nn.Module):
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        ...
        x = self.token_emb(motion_ids) # (b, seqlen) -> (b, seqlen, d)
        # print(x.shape)
        ...
        logits = self.output_process(output) #(seqlen, b, e) e.g. (49, b, 384) -> (b, ntoken, seqlen) e.g. (b, 512, 49)
        return logits
    ...
    def forward(self, ids, y, m_lens):
        ...
        num_token_masked = (ntokens * rand_mask_probs).round().clamp(min=1) # (bs,) The number of tokens to be masked in each sequence.

        # A tensor where each row contains the indices of a randomly shuffled sequence of token positions. 
        # This is used to randomly choose which tokens to mask.
        batch_randperm = torch.rand((bs, ntokens), device=device).argsort(dim=-1)
        
        # Tokens whose shuffled positions in batch_randperm are less than num_token_masked are marked as True (to be masked)
        ...
        labels = torch.where(mask, ids, self.mask_id) #(masked positions hold the original token values, and unmasked positions are replaced by self.mask_id)

        x_ids = ids.clone()

        # Further Apply Bert Masking Scheme
        # Step 1: 10% replace with an incorrect token
        mask_rid = get_mask_subset_prob(mask, 0.1) # from those selected to be masked, 10% are selected to be replaced by a random token
...
class ResidualTransformer(nn.Module):
    ...
    def output_project(self, logits, qids):
        '''
        projects the output logits from the transformer layers into token space
        ...
        output_proj_weight = self.output_proj_weight[qids] # used to map the logits to the token space
    ...
    def forward(self, all_indices, y, m_lens):
        ...
        token_embed = repeat(self.token_embed_weight, 'q c d-> b c d q', b=bs)  #each batch item gets a copy of the token embedding.
        # gather the embeddings from the token_embed matrix using the quantized token indices stored in all_indices.
        gather_indices = repeat(all_indices[..., :-1], 'b n q -> b n d q', d=token_embed.shape[2])
        # print(token_embed.shape, gather_indices.shape)
        # uses the indices in gather_indices to look up the corresponding embeddings in the token_embed matrix.
