...
def top_k(logits, thres = 0.9, dim = 1):
    """
    Selects the top-k values from the `logits` tensor based on the given threshold `thres` and replaces all other values with negative infinity (-inf). 
    The top-k values are determined along the specified dimension `dim`.
    Returns:
    --------
    probs : torch.Tensor
        A tensor of the same shape as `logits`, where only the top-k values are retained, and all other values are replaced with `-inf`. The retained values maintain their original positions in the tensor.
    Example:
    --------
    >>> logits = torch.tensor([[0.1, 0.4, 0.9, 0.2],
                               [0.7, 0.6, 0.3, 0.5]])
    >>> top_k(logits, thres=0.5)
    tensor([[-inf, 0.4000, 0.9000, -inf],
            [ 0.7000, 0.6000, -inf, -inf]])
    Notes:
    ------
    The number of top-k values retained is computed as:
        k = ceil((1 - thres) * logits.shape[dim])
    This ensures that at least one value is kept for very small thresholds.
    """
    ...
    # func verified
    # non_inf_count_per_row = torch.sum(~torch.isinf(probs[0]), dim=1)
    # min_non_inf_count = torch.min(non_inf_count_per_row)
    return probs
