...
class MaskTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        if 'Conditional' in self.vq_model.__class__.__name__:
            input = (motion, label.to(self.device))
        else:
            input = (motion,)
        code_idx, _ = self.vq_model.encode(*input)
...
class ResidualTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        # (b, n, q), (q, b, n ,d)
        # (b, n(number of codebooks), q(6layers)), (b, n:49, q, d:code_dim)
        code_idx, all_codes = self.vq_model.encode(*input)
        m_lens = m_lens // 4
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching = evaluation_res_transformer(
