...
class ResidualVQ(nn.Module):
    ...
    def forward(self, x, return_all_codes = False, sample_codebook_temp = None, force_dropout_index=-1, num_layers=None):
        # debug check
        ...
        for quantizer_index, layer in enumerate(self.layers):
            if num_layers is not None and quantizer_index >= num_layers:
                break
    ...
    def quantize(self, x, return_latent=False):
        ...
        code_idx = torch.stack(all_indices, dim=-1) #(bs, seq_len/4(#codebooks assigned), num_quantizers(6))
        all_codes = torch.stack(all_codes, dim=0) #(num_quantizers, bs, code_dim, seq_len/4)
        if return_latent:
